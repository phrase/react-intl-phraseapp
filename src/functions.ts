...
export function initializePhraseAppEditor(config: any) {
  ...
  if (typeof window === 'undefined') {
    (<any>global).PHRASEAPP_ENABLED = config.phraseEnabled;
    (<any>global).PHRASEAPP_CONFIG = sanitizeConfig(config);
  } else {
...
export function isPhraseEnabled(): boolean {
  return (typeof window === 'undefined') ? (<any>global).PHRASEAPP_ENABLED : (<any>window).PHRASEAPP_ENABLED
}

export function escapeId(id: string): string {
  const config = (typeof window === 'undefined') ? (<any>global).PHRASEAPP_CONFIG : (<any>window).PHRASEAPP_CONFIG;
